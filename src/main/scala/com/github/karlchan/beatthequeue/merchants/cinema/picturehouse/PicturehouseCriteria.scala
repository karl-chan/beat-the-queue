...
final case class PicturehouseCriteria(
...
) extends Criteria[Picturehouse]:
  def matches(event: Event[Picturehouse]) =
    val PicturehouseEvent(_, name, time, venue, screenType) =
      event.asInstanceOf[PicturehouseEvent]
