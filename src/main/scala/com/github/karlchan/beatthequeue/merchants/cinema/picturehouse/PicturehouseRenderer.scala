...
class PicturehouseRenderer
    extends Renderer[Picturehouse, PicturehouseCriteria, PicturehouseEvent]:

  private val cachedInfo: IO[PicturehouseCrawler#Info] =
    PicturehouseCrawler().getInfo().memoize.unsafeRunSync()

  override def toFields(criteria: PicturehouseCriteria) = Seq(
  ...
  override def toInputFields(criteria: PicturehouseCriteria) =
    for {
      info <- cachedInfo
    } yield Seq(
  ...
  override def toFields(event: PicturehouseEvent) = Seq(
